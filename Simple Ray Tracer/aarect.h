...
bool xy_rect::hit(const ray& r, float t0, float t1, hit_record& rec) const {
	float t = (k - vec3::ToFloat(r.origin().z())) / vec3::ToFloat(r.direction().z());
	if (t < t0 || t > t1)
		return false;
	float x = vec3::ToFloat(r.origin().x() + t * r.direction().x());
	float y = vec3::ToFloat(r.origin().y() + t * r.direction().y());
...
bool xz_rect::hit(const ray& r, float t0, float t1, hit_record& rec) const {
	float t = (k - vec3::ToFloat(r.origin().y())) / vec3::ToFloat(r.direction().y());
	if (t < t0 || t > t1)
		return false;
	float x = vec3::ToFloat(r.origin().x() + t * r.direction().x());
	float z = vec3::ToFloat(r.origin().z() + t * r.direction().z());
...
bool yz_rect::hit(const ray& r, float t0, float t1, hit_record& rec) const {
	float t = (k - vec3::ToFloat(r.origin().x())) / vec3::ToFloat(r.direction().x());
	if (t < t0 || t > t1)
		return false;
	float y = vec3::ToFloat(r.origin().y() + t * r.direction().y());
	float z = vec3::ToFloat(r.origin().z() + t * r.direction().z());
